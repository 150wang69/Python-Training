...
def create_list(data):
    ...
    with open("bilibili.json", "a")as f:
        f.write(js + '\r\n')
...
def show(data):
    ...
    for k in data:
        tmp["aid"].append((k["aid"]))
        tmp["view"].append((k["view"]))
    df = pd.DataFrame(tmp)
    df.plot(kind='bar', x='aid', y='view')
...
def open_file_json():
    data = []
    with open("bilibili.txt", "r") as file:
        for line in file:
...
for k in data:
    dt = datetime.strptime(k["now"], "%Y-%m-%d %H:%M:%S")
    tmp["time"].append(dt.strftime("%M:%S"))
    for kk in k["list"]:
        if kk["aid"] == 626357031:
            tmp["value"].append(kk["view"])
